...
//        person = personRepo.findOne("admin");
    ...
    public void clearhistoryTest(){
        ...
        sessionRepo.updateSession(request, null);
        queueService.joinQueue(session, "Salla", "Moikkis");
        MsgToServer msg = MsgToServer.create("x", channelId, "haloo");
        chatLogger.logNewMessage(msg);
        adminService.clearHistory();
        Assert.assertNull(sessionRepo.getSessionFromUserId("x"));
        Assert.assertNull(sessionRepo.getSessionFromSessionId("sessionx"));
